...
async function aggregationQueries() {
  try {
    ...
    const presentationsWithMostSlides = presentationsWithSlideCounts.filter(
      p => p._count.eqSlides === maxSlideCount
    );
...
if (require.main === module) {
  runAllExamples()
    .then(() => console.log('Query examples execution completed.'))
    .catch(error => {
      console.error('Unhandled error during example execution:', error);
      process.exit(1);
    });
}
  ...
  main()
    ...
    .catch((e) => {
      console.error('Unhandled error during import:', e);
      process.exit(1);
    });
///////////////////////////////////